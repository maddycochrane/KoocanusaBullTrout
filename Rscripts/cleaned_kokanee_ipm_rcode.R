...
### compare model estimate to observed ata


Nadult.low<-kok.ipm.mod.dd$BUGSoutput$summary[1:44,3] # 2.5% 
Nadult.high<-kok.ipm.mod.dd$BUGSoutput$summary[1:44,7] # 97.5%
Nadult<-kok.ipm.mod.dd$BUGSoutput$summary[1:44,1] # mean


#q.lake.kok[t] <- meanq*exp(length[t]*beta.length )
length = c(kok.meanL.new$meanL_scale[2:45])
q.lake.kok <- kok.ipm.mod.dd$BUGSoutput$mean$meanq * exp(length*kok.ipm.mod.dd$BUGSoutput$mean$beta.length)

mean.expansion <- kok.ipm.mod.dd$BUGSoutput$mean$mean.expansion 

kok.df<-data.frame(kokAdult = Nadult, kokAdult.low = Nadult.low, kokAdult.high = Nadult.high, 
                   yrs = seq(1980,2023,1),
                   kokAdult.scaled = q.lake.kok*Nadult,
                   kokAdult.low.scaled = q.lake.kok*Nadult.low, 
                   kokAdult.high.scaled = q.lake.kok*Nadult.high ,
                   obs.redds = colSums(  kokanee.redds.wide[,6:49]))
tail(kok.df)

# hydro-acoustic data 
kok.hydro.long %>% filter(Group == "Adult")%>%
ggplot(aes(x=YYYY,y=Redds))+
  scale_x_continuous(limits=c(2013,2023),breaks=c(2013,2015,2017,2019,2021,2023))+
  labs(y="Adult kokanee (n)",x="Year")+theme_minimal()+
  
  geom_point(data=kok.df, aes(x=yrs, y = Nadult),col="grey")+
  geom_line(data=kok.df, aes(x=yrs, y = Nadult),col="grey")+
...
# kokanee cpue data
  ggplot()+geom_boxplot(data = kok.net.sums , aes(x=YYYY,y=cpue,group=YYYY))+
  scale_y_continuous(limits=c(0,60),expand=c(0,0))+
  scale_x_continuous(limits=c(1979,2024), minor_breaks = seq(1980,2022,2),
                     breaks=seq(1980,2022,4),expand=c(0,0))+
  labs(x="Year",y="Relative adult kokanee abundance")+theme_minimal()+

  geom_point(dat=kok.df,aes(x=yrs, y=kokAdult.scaled),col="darkgrey")+
  geom_ribbon(dat=kok.df, aes(x=yrs, ymin = kokAdult.low.scaled, 
                             ymax = kokAdult.high.scaled),alpha=0.1)+
  geom_line(dat=kok.df,aes(x=yrs, y=kokAdult.scaled),col="darkgrey")+
  ...
  # kokanee redd data 
  ggplot(dat=kok.df, aes(x=yrs, y=kokAdult*mean.expansion))+geom_point(col="darkgrey")+
    geom_line(col="darkgrey")+
    geom_ribbon(dat=kok.df, aes(x=yrs, ymin = kokAdult.low*mean.expansion, 
                                ymax = kokAdult.high*mean.expansion),alpha=0.1)+
    
    geom_point(data=kok.df,aes(x=yrs, y=obs.redds),col="black")+
    geom_line(data=kok.df,aes(x=yrs, y=obs.redds),col="black")+
    
    theme_minimal()+labs(x="Year",y="Kokanee escapement")+
    annotate("text",x=1985,y=3e+05,label="Model estimate",col="grey")+
    annotate("text",x=2005,y=4e+05,label="Observed redds")+
    theme(axis.text.x=element_text(angle=45,hjust=1),text = element_text(size = 12),
          plot.background = element_rect(fill = 'white', colour = 'white'))
...
################################################################
## average weight by length ### length-weight relationship
...
#### bring in new length data for small kokanee
kok.length.new<-read.csv("Data/2017 Verticle net kokanee length weight.csv",header=TRUE) 
head(kok.length.new)
kok.length.new <- kok.length.new %>% rename(Length_mm = Lenth)%>%rename(Weight_g = Weight)%>%
  dplyr::select(Species,Length_mm, Weight_g)

kok.length.new2<-full_join(kok.raw.net.dat, kok.length.new,by=c("Length_mm","Weight_g"))
tail(kok.length.new2)

kok.length.new2 <- kok.length.new2 %>% 
  mutate(est.age = case_when(Length_mm < 150 ~ 1, 
                             Length_mm < 230 ~ 2, 
                             Length_mm < 280 ~ 3,
                             Length_mm >279 ~ 4))

# fit length-weight model
nls_fit2 <- nls(formula = Weight_g ~ a*Length_mm^b, 
               start = c(a=6e-06, b=3), data = kok.length.new2)
coef(nls_fit2) # more or less the same as earlier dat

ggplot(data=kok.length.new2, aes(x= Length_mm,y=Weight_g,color=as.factor(est.age)))+geom_point()+
  stat_smooth(method = 'nls', formula = 'y~a*x^b', method.args = list(start= c(a = 6e-06,b=3)),se=FALSE)+
  scale_y_continuous(expand=c(0,0),limits=c(0,1260))+
  labs(x="Length (mm)", y="Weight (g)")+theme_bw()

ggplot(data=kok.length.new2, aes(x= Length_mm,y=Weight_g))+geom_point()+
  stat_smooth(method = 'nls', formula = 'y~a*x^b', method.args = list(start= c(a = 6e-06,b=3)),se=FALSE)+
  scale_y_continuous(expand=c(0,0),limits=c(0,1260))+
  labs(x="Length (mm)", y="Weight (g)")+theme_bw()+
  annotate(geom="text", x=200, y=1000, label = "y == 5.3e-06*x^{3.10}", parse =TRUE)
...
kok.length.new2 %>% mutate(relative.year = if_else(YYYY<2000,"early","late"))%>%drop_na(YYYY)%>%
ggplot( aes(x= Length_mm,y=Weight_g,group=relative.year,color=as.factor(relative.year)))+
  geom_point()+facet_wrap(~relative.year)+
  stat_smooth(method = 'nls', formula = 'y~a*x^b',
              method.args = list(start= c(a = 6e-06,b=3)),se=FALSE)+
  scale_y_continuous(expand=c(0,0),limits=c(0,1260))+
  labs(x="Length (mm)", y="Weight (g)")+theme_bw()+theme(legend.position = "none")


# mean length of adults 
head(kok.meanL.new)
kok.biomass.chk <- kok.meanL.new %>% dplyr::select(YYYY, meanL)%>%rename(adult.meanL = meanL)%>%
  mutate(sub.adult.meanL = adult.meanL-51)%>%
  mutate(age1.meanL = sub.adult.meanL-71)%>%
  mutate(age0.meanL = age1.meanL-71)%>%
  mutate(adult_ag_weight = 5.272309e-06*(adult.meanL)^3.101708)%>%
  mutate(subadult_ag_weight = 5.272309e-06*(sub.adult.meanL)^3.101708)%>%
  mutate(age1_ag_weight = 5.272309e-06*(age1.meanL)^3.101708)%>%
  mutate(age0_ag_weight = 5.272309e-06*(age0.meanL)^3.101708)
head(kok.biomass.chk)
  
  
tail(kok.hydro.acoustic)


kok.hydro.acoustic$Age1.and0.Kokanee <- as.integer(kok.hydro.acoustic$Age1.and0.Kokanee )
kok.hydro.acoustic$Age2.Kokanee <- as.integer(kok.hydro.acoustic$Age2.Kokanee )
kok.hydro.acoustic$Age3.Kokanee <- as.integer(kok.hydro.acoustic$Age3.Kokanee )
kok.hydro.acoustic%>% dplyr::select(YYYY, Age3.Kokanee, Age2.Kokanee, Age1.and0.Kokanee)%>%
  pivot_longer(cols=2:4,names_to="stage")%>%
  mutate(stage = case_when(stage == "Age3.Kokanee" ~ 3,
                           stage == "Age2.Kokanee" ~ 2,
                           stage == "Age1.and0.Kokanee"~ 1))%>%
  ggplot( aes(x=YYYY,y=value,group=as.factor(stage),fill=as.factor(stage)))+geom_col()

kok.hydro.acoustic%>%drop_na(Age3.Kokanee)%>%
  mutate(totalCount = Age3.Kokanee+Age2.Kokanee+Age1.and0.Kokanee)%>%
  mutate(Age3.prop = Age3.Kokanee/totalCount)%>%
  mutate(Age2.prop = Age2.Kokanee/totalCount)%>%
  mutate(Age1.prop = Age1.and0.Kokanee/totalCount)%>%
  summarise(mean3 = mean(Age3.prop), mean2 = mean(Age2.prop),mean1 = mean(Age1.prop))
# 70% to 19% to 11%  



kok.counts <- kok.hydro.acoustic %>% dplyr::select(YYYY,Age3.Kokanee, Age2.Kokanee, Age1.and0.Kokanee)%>% 
  filter(YYYY>2012)

kok.biomass.chk<-left_join(kok.counts, kok.biomass.chk)
str(kok.biomass.chk)
kok.biomass.chk$Age1.and0.Kokanee <- as.integer(kok.biomass.chk$Age1.and0.Kokanee)


kok.biomass.chk <- kok.biomass.chk%>%
  mutate(adultBiomass = Age3.Kokanee*adult_ag_weight)%>%
  mutate(subadultBiomass = Age2.Kokanee*subadult_ag_weight)%>%
  mutate(age1Biomass = (0.3*Age1.and0.Kokanee)*age1_ag_weight)%>%
  mutate(age0Biomass = (0.7*Age1.and0.Kokanee)*age0_ag_weight)%>%
  mutate(age0and1Biomass = age1Biomass+age0Biomass)%>%
  dplyr::select(-age1Biomass,-age0Biomass)%>%
  mutate(totalBiomass = adultBiomass+subadultBiomass+age0and1Biomass)%>%
  mutate(age1Proportion = age0and1Biomass/totalBiomass)


ggplot(data=kok.biomass.chk, aes(x=YYYY,y=age1Proportion))+geom_col()
kok.biomass.chk %>% summarise(mean=mean(age1Proportion),sd=sd(age1Proportion),
                              max=max(age1Proportion),min=min(age1Proportion))
# average 18% of total biomass, sd = 12 (range 3-45%)


kok.biomass.chk %>% dplyr::select(YYYY, adultBiomass, subadultBiomass, age0and1Biomass)%>%
  pivot_longer(cols=2:4,names_to="stage")%>%
  mutate(stage = case_when(stage == "adultBiomass" ~ 3,
                           stage == "subadultBiomass" ~ 2,
                           stage == "age0and1Biomass"~ 1))%>%
ggplot( aes(x=YYYY,y=value,group=as.factor(stage),fill=as.factor(stage)))+geom_col()

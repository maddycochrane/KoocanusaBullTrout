...
long.kok.spawn<-kok.spawn.counts%>%
  gather(key=Site, value= Count, SandCreek:Total, factor_key=TRUE)
head(long.kok.spawn)
...
p3<-ggplot(data=long.kok.spawn)+
  geom_point(aes(x=Year,y=Count))+
  geom_line(aes(x=Year,y=Count))+
  labs(y="Kokanee Escapement")+
  scale_color_grey()+theme_classic()+facet_wrap(~Site,nrow=2)
p3
...
bt1<-bt.redd.counts %>%
  ...
  filter(LocalPopulation != "Wigwam River")%>%
ggplot()+
  geom_point(aes(x=Year,y=Redds,color=LocalPopulation))+
  geom_line(aes(x=Year,y=Redds,color=LocalPopulation),alpha=0.2)+
...
# save figure
tiff("Results/bt.redd.counts.tiff",width = 6, height = 6, units = 'in', res = 300)
duplot
dev.off()
